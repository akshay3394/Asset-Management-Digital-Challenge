...
public class AccountsServiceTest {
  ...
  @Test
  public void getAccountDoesNotExist() throws Exception{
    String uniqueId = "Id-" + System.currentTimeMillis();
    try {
  ...
  public void transferAmount() throws Exception{
    ...
    this.accountsService.amountTransfer(new AmountTransferRequest(fromAccountId, toAccountId, new BigDecimal(100)));
  ...
  public void transferAmountAmountMoreThanBalance() throws Exception{
    ...
    try {
      this.accountsService.amountTransfer(new AmountTransferRequest(fromAccountId, toAccountId, new BigDecimal(10000)));
    }catch (InsufficientAccountBalanceException insufficientAccountBalanceException){
      assertThat(insufficientAccountBalanceException.getMessage()).isEqualTo("Insufficient account balance in accountId:"+fromAccountId+"to perform this transaction");
    }
  ...
  public void transferAmountNegativeAmount() throws Exception{
    ...
    try {
      this.accountsService.amountTransfer(new AmountTransferRequest(fromAccountId, toAccountId, new BigDecimal(-1)));
    }catch (InsufficientAccountBalanceException insufficientAccountBalanceException){
      assertThat(insufficientAccountBalanceException.getMessage()).isEqualTo("Insufficient account balance in accountId:"+fromAccountId+"to perform this transaction");
    }
  ...
  public void transferAmountCheckNotification() throws Exception{
    ...
    this.accountsService.amountTransfer(new AmountTransferRequest(fromAccountId, toAccountId, amountToTransfer));
